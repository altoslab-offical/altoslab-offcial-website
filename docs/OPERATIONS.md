...
- `scripts/blog-scheduled-runner.mjs --backfill --target-posts 40` checks the live public inventory per configured language. The `40` target means every language must reach 40 posts, not that the whole site has 40 posts total. The planner calculates one complete 9-language article set for each missing post in the lowest-coverage language and writes a fail-closed queue under `data/blog-backfill/<date>/`. The queue alternates market-news and column lanes, but it only creates browser-production prompt cards and held manifests. It does not publish until Gemini/GPT or source-image evidence, local preflight, validate-only, image QA, multilingual parity and design QA all pass.
