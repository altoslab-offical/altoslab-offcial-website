...
- The source registry controls the current production target: 31 `breaking` market-news posts and 9 `column` posts per configured language. Breaking posts prioritize latest official/trusted news and preserve source style; columns are one Gemini-approved original ALTOS LAB argument per Taipei day. `feature` remains a supported schema type, but it is not part of the current routine production mix.
...
- `scripts/blog-scheduled-runner.mjs --backfill --target-posts 40` checks the live public inventory per configured language. The `40` target means every language must reach 40 posts, not that the whole site has 40 posts total. The planner calculates one complete 9-language article set for each missing post in the lowest-coverage language and writes a fail-closed queue under `data/blog-backfill/<date>/`. The queue alternates market-news and column lanes, but it only creates prompt cards and held manifests. It does not publish until lane-specific evidence passes: source-translation/source-image for market news, Gemini/GPT evidence for columns/features, plus local preflight, validate-only, image QA, multilingual parity and design QA.
