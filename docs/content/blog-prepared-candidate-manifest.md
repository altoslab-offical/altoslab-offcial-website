...
- For market-news sets where every post is `breaking` and `generation.provider` is `source-translation`, Gemini and ChatGPT evidence are not required; the hard gate is source fidelity, all-language localization, and accepted shared source/official cover metadata. Approved ALTOS LAB editorial fallback covers are not accepted for market news production release.
...
- Market-news covers are credited source article or official announcement images. Never substitute stock/free images, local placeholders, GPT art, manual fallback covers, or generated fallback covers.
