...
- 專欄翻譯不可把專業詞硬直譯成卡讀者的標題詞。像 `red team` 若用在 zh-Hant 標題，優先改成讀者懂的「壓力測試 / 採購壓力測試」，首次出現再解釋它和資安紅隊的關係。標題、excerpt、H2 與圖說要用當地語言收尾；zh-Hant/ja/ko/th 文章的 public image caption 不可整句英文。
- 已發布內容修復不能只看 `/api/blog`。API readback 新不代表 article HTML 已新；zh-Hant detail page 曾因 `react cache()` 黏住舊稿。發布後要同時驗 `/api/blog/[slug]`、HTML、generated-media HEAD/GET、Chrome extension rendered DOM 與底部 related cards。若 HTML 仍舊，先 refresh public projection；若 process memory 仍舊，deploy/force ECS new task。
- 同站 `/api/blog/generated-media/*` 是我們能控制的 durable media，release-set 與 bulk-patch 都必須 probe reachability。404、410、非 image content-type 或過小檔案是 hard block；不能讓候選只因 JSON 裡有 URL 就進 production。
...
- Legacy/static articles can bypass CMS mutation paths. If signed `bulk-patch` says "not found" but `/api/blog` still serves the post, search `data/static-blog-overrides.json` and other static projections before assuming CMS corruption. Static override repairs require code commit, AWS image rebuild, ECS deploy, and production generated-media scan after deploy.
...
- Blog publish/refresh 不能只刷新 S3/CMS public projection。每次 `release-set` publish 或 `refresh-public-cache` 成功後，必須同步 `revalidatePath` blog indexes、article detail、RSS、sitemap、`llms.txt`，否則會出現 `/api/blog` 已更新但 `/blog` HTML/Chrome 仍顯示舊文章數的分岔。完成證據要包含 API count、detail readback、`/blog` HTML 或 Chrome rendered count。
- `refresh-public-cache` response 的 `languages` 是 canonical CMS count；前台 effective count 要看 `effectiveLanguages`，因為 public read path 會 merge static/backfill overrides。Hermes 判斷 Blog 是否真的新增文章時，以 `effectiveLanguages` + `/api/blog` + Chrome rendered count 為準。
- Static legacy overrides 是另一條修復路徑。若 Chrome rendered QA 看到破圖或 client-blocked 圖，但 `HEAD/GET` 回 200，先檢查 `data/static-blog-overrides.json` 是否仍引用舊 `browser-gemini-gpt` 檔名。這類修復要換成中性 generated-media 檔名、更新 static override、commit/push、AWS ECS deploy，再用 Chrome rendered DOM 驗證；signed CMS `bulk-patch` 不會更新 static override 文章。
- AWS ECS deploy 若卡在 root MFA device registration，這是帳號安全設定而不是 code/deploy script 問題。不要替使用者建立 MFA 裝置或繞過；先保留 GitHub commit 與本地測試證據，等 Tommy 完成 root MFA 後再跑 `aws login --profile altoslab`、ECR push、ECS update、production smoke。
...
- 2026-06-24 daily target miss 的根因是三個 column slot 都停在 `awaiting_browser_production`，沒有產出 `article-set.json`。修復規格：排程可設定 `ALTOS_BLOG_AUTO_CODEX_COLUMN_PRODUCER=1`，讓 `scripts/blog-scheduled-runner.mjs` 在 prep 後呼叫 `scripts/blog-codex-column-producer.mjs` 產生 9 語 article set 與 `codexEvidence`。但正式發布必須帶入已審核 GPT image2 visual packet；本地 Pillow/SVG/抽象卡片圖只能作 debug，不能 publish。後續仍要 `column-validate -> release -> verify-blog-release -> daily-closeout` 全部通過。
- 這次 gate 學到的品質規則要寫回 Hermes：不要用「不是 X，而是 Y」當開場骨架；九語言不可混英文 source lead；中日韓要有足夠知識密度；generated media 低於 64KB 會在 release reachability 被擋；品質不合格時要 repair 到 pass，不准把 `held` 當成成功 skip。
