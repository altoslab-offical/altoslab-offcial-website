...
- Blog UI Stability Contract: n8n must not change `/blog`, `/:language/blog`, `components/BlogIndex.tsx`, route ownership, or visual hierarchy. The protected index markers are `blog-journal-index`, `blog-index-hero`, `blog-lane-strip`, `blog-index-toolbar`, `blog-index-grid`, and `blog-card`; n8n must not reintroduce `blog-craft-sidebar`, visible `AI & Craft` sidebar copy, `BlogIndexLite`, or `blog-lite-*` emergency UI.
...
curl -fsS -X POST -H "content-type: application/json" --data '{"source":"manual-smoke"}' http://127.0.0.1:5679/webhook/altos-blog/manual/daily-closeout
