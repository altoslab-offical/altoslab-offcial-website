...
const BLOG_AD_SLOT_ENV_KEYS = {
  "after-summary": "NEXT_PUBLIC_ADSENSE_BLOG_AFTER_SUMMARY_SLOT",
  "mid-article": "NEXT_PUBLIC_ADSENSE_BLOG_MID_ARTICLE_SLOT",
  "before-related": "NEXT_PUBLIC_ADSENSE_BLOG_BEFORE_RELATED_SLOT"
};

function isBlogAdsenseEnabled(env) {
  const raw = String(env.NEXT_PUBLIC_ADSENSE_BLOG_ADS_ENABLED || "").trim().toLowerCase();
  return raw ? !["0", "false", "off", "disabled", "no"].includes(raw) : true;
}

function blogAdSlotHtml(env, placement) {
  if (!isBlogAdsenseEnabled(env)) return "";
  const client = String(env.NEXT_PUBLIC_ADSENSE_CLIENT || "").trim();
  const slot = String(env[BLOG_AD_SLOT_ENV_KEYS[placement]] || "").trim();
  if (!/^ca-pub-\d+$/i.test(client) || !/^\d+$/.test(slot)) return "";
  return `<aside class="blog-adsense-slot is-${escapeAttribute(placement)}" data-blog-ad-placement="${escapeAttribute(placement)}" aria-label="Advertisement">
    <ins class="adsbygoogle" style="display:block" data-ad-client="${escapeAttribute(client)}" data-ad-slot="${escapeAttribute(slot)}" data-ad-format="auto" data-full-width-responsive="true"></ins>
...
<div class="site-home blog-site-shell">
  ...
  <main class="blog-page blog-article-page">
    <article>
      ...
      ${blogAdSlotHtml(env, "before-related")}
    </article>
  </main>
