...
function renderIndex(posts, language, env, requestUrl) {
  ...
  const pageSize = 18;
  const filtered = posts.filter((post) => topicMatches(post, tag) && queryMatches(post, query));
...
function renderLlms(posts, env, requestUrl) {
  ...
  const lines = [
    ...
    "",
    "## Blog",
    ...posts.slice(0, 60).map((post) => `- [${plainText(post.title)}](${siteUrl}${blogPostPath(post)}): ${plainText(post.excerpt || post.seoDescription || "").slice(0, 180)}`)
  ];
  return `${lines.join("\n")}\n`;
}
