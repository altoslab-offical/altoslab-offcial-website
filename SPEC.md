...
WonDa runtime contract (2026-07-29): the existing public Widget placement and visual behavior stay unchanged, while every homepage, App Router, and direct Cloudflare blog render surface must use the same approved ALTOS LAB public demo channel. AWS/custom-domain and Vercel backup embeds must each use their own public website `/api/wonda` proxy so the same multilingual and provider-leak guard runs before forwarding to the matching WonDa production API; no demo login credential, JWT, or model key may enter public source.
...
- Article cover images must keep a stable 1200:630 display ratio, `object-fit: cover`, centered crop, and compact hero spacing so source/generated images do not create oversized blanks or distorted article openings.
...
- Cloudflare article deploys must verify the built CSS artifact, not only source files: article title CSS stays at the reduced `clamp(34px, 3.25vw, 44px)` scale, takeaway emphasis uses inline signal-lime text treatment, and stale oversized-title or purple-accent chunks must fail preflight.
...
- The direct Worker renderer and `/api/blog-html` renderer must both use a Blog-main-style `siteHeaderHtml(...)` helper. Do not reintroduce the older standalone `.nav` / `.langs` article header bar.
