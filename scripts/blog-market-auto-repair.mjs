...
function repairAntiSlop(text = "") {
  return cleanText(text)
    .replace(/\bnearly every\b/gi, "almost every")
    .replace(/\bautonomously execute\b/gi, "execute on its own")
    .replace(/\bautonomously\b/gi, "on its own")
    .replace(/\bshowed up clearly\b/gi, "showed up in a concrete way")
    .replace(/\bclearly\b/gi, "in concrete terms")
    .replace(/\bexclusively used\b/gi, "used only")
    .replace(/\bexclusively\b/gi, "only")
    .replace(/\bentirely sure\b/gi, "sure")
    .replace(/\bentirely by myself\b/gi, "on my own")
    .replace(/\bentirely by\b/gi, "by")
    .replace(/\bentirely\b/gi, "in full")
    .replace(/\bclosely enough\b/gi, "well enough")
