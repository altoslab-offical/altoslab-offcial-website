...
export function cleanMarketPublicText(value = "", language = "") {
  ...
  if (language === "zh-Hant") {
    text = text
      ...
      .replace(/出於某種原因，?/g, "")
      .replace(/Warner Bros[。．]\s*旗下/g, "Warner Bros. 旗下")
      .replace(/Warner Bros[。．](?=\s|$|，|、)/g, "Warner Bros.")
      .replace(/Netflix、A24、Focus Features 和 Warner Bros[。.]?/g, "Netflix、A24、Focus Features 與 Warner Bros. 旗下 Clockwork")
      .replace(/Netflix、A24、Focus Features 與 Warner Bros\.(?!\s*旗下 Clockwork)/g, "Netflix、A24、Focus Features 與 Warner Bros. 旗下 Clockwork")
      .replace(/報導指出，?《Clockwork》都決定不再接手/g, "報導指出，Netflix、A24、Focus Features 與 Warner Bros. 旗下 Clockwork 都決定不再接手")
      .replace(/工作室不願收購/g, "片商不願接手")
      .replace(/盧卡·瓜達尼諾/g, "Luca Guadagnino")
      .replace(/導演Luca/g, "導演 Luca")
      .replace(/Luca Guadagnino\s*\(Luca Guadagnino\)/g, "Luca Guadagnino")
      .replace(/接手([^。！？\n]{0,90})，洽談發行/g, "接手$1的發行")
      .replace(/\.\s+(?=[A-Z\u4e00-\u9fff])/g, "。")
      .replace(/Warner Bros。/g, "Warner Bros.")
      .replace(/Warner Bros\.\s*旗下/g, "Warner Bros. 旗下")
      .replace(/Warner Bros\.\s*旗下 Clockwork。Neon/g, "Warner Bros. 旗下 Clockwork 都暫不接手；Neon")
      .replace(/Warner Bros\.\s*旗下 Clockwork。/g, "Warner Bros. 旗下 Clockwork。")
      .replace(/Netflix、A24、Focus Features 與 Warner Bros\.\s*旗下 Clockwork。Neon/g, "Netflix、A24、Focus Features 與 Warner Bros. 旗下 Clockwork 都暫不接手；Neon")
      .replace(/\s+([，。；：！？])/g, "$1")
      .replace(/([。！？])[ \t]+/g, "$1")
      .replace(/([（「])[ \t]+/g, "$1")
      .replace(/[ \t]+([）」])/g, "$1");
...
function knownProfile(article = {}, frame = {}) {
  ...
  if (/hollywood.*bending.*knee.*openai|luca-guadagnino-artificial-sam-altman|artificial.*sam altman/.test(text)) {
    return {
      title: {
        "zh-Hant": "好萊塢片商暫不接手 OpenAI 傳記電影《Artificial》",
        en: "Hollywood studios hesitate over the OpenAI biopic Artificial",
        ja: "OpenAI 伝記映画『Artificial』にハリウッド各社が慎重姿勢",
        ko: "할리우드 스튜디오들, OpenAI 전기 영화 Artificial 인수에 신중",
        id: "Studio Hollywood menahan diri dari biopik OpenAI Artificial",
        vi: "Các hãng phim Hollywood thận trọng với phim tiểu sử OpenAI Artificial",
        th: "สตูดิโอฮอลลีวูดยังลังเลกับหนังชีวประวัติ OpenAI เรื่อง Artificial",
        ms: "Studio Hollywood berhati-hati terhadap biopik OpenAI Artificial",
        fil: "Nag-aalangan ang Hollywood studios sa OpenAI biopic na Artificial"
      },
      standfirst: {
        "zh-Hant": "The Verge 報導，Netflix、A24、Focus Features 與 Warner Bros. 旗下 Clockwork 都暫不接手 Luca Guadagnino 執導的 OpenAI 傳記電影《Artificial》，Neon 和 Mubi 據稱仍有興趣。",
        en: "The Verge reports that Netflix, A24, Focus Features, and Warner Bros.-owned Clockwork have passed on Luca Guadagnino's OpenAI biopic Artificial, while Neon and Mubi are still said to be interested."
      },
      facts: {
        "zh-Hant": [
          "The Verge 報導，Netflix、A24、Focus Features 與 Warner Bros. 旗下 Clockwork 都暫不接手 Luca Guadagnino 執導的《Artificial》。",
          "Neon 和 Mubi 據稱仍然對這部聚焦 OpenAI 與 Sam Altman 的電影感興趣。",
          "Amazon MGM 上週突然放棄發行計畫；報導稱電影原本準備走奧斯卡資格短期院線、2027 年初更廣泛上映，以及 SXSW 放映。",
          "The Verge 將 Amazon 的決定放在其今年稍早對 OpenAI 投資 500 億美元的脈絡中觀察。",
          "報導的核心不是電影已取消，而是大型片商對批判 AI 產業題材的發行風險變得更謹慎。"
        ],
        en: [
          "The Verge reports that Netflix, A24, Focus Features, and Warner Bros.-owned Clockwork have passed on Luca Guadagnino's Artificial.",
          "Neon and Mubi are still said to be interested in the film about OpenAI and Sam Altman.",
          "Amazon MGM unexpectedly dropped its release plans while post-production was close to completion.",
          "The film had reportedly been aimed at a short Oscar-qualifying theatrical run, a wider early-2027 release, and an SXSW screening.",
          "The Verge frames Amazon's move against its earlier $50 billion OpenAI investment and Hollywood's caution around critical big-tech stories."
        ]
      }
    };
  }
...
function usableExistingMarketTitle(value = "") {
  ...
  if (/^[A-Za-z][^「"]{0,120}(?:發布|發佈|公開|報導|報道)「[^」]*[A-Za-z]{4,}[^」]*」/i.test(text)) return false;
  if (/^[\w &/.-]{2,80}\s*(?:發布|發佈|公開|報導|報道)「[^」]*[A-Za-z]{4,}[^」]*」/i.test(text)) return false;
  if (/(?:發布|發佈|公開|報導|報道)「[^」]*[A-Za-z]{4,}\s+[A-Za-z]{4,}[^」]*」/i.test(text)) return false;
...
function splitReadableSentences(value = "") {
  ...
  const abbreviationDot = "__ALTOS_ABBREVIATION_DOT__";
  const text = normalizeNewsText(value)
    .replace(/(\d)\.(\d)/g, `$1${decimalDot}$2`)
    .replace(/\b(Warner Bros|Mr|Mrs|Ms|Dr|Prof|Inc|Corp|Co|Ltd|St)\./g, (_, prefix) => `${prefix}${abbreviationDot}`)
    .replace(/\b(U\.S|U\.K|E\.U)\./g, (match) => match.replaceAll(".", abbreviationDot));
  const matches = text.match(/[^。！？.!?]+[。！？.!?]?/g) || [];
  return matches
    .map((sentence) => sentence.replaceAll(decimalDot, ".").replaceAll(abbreviationDot, ".").trim())
    .filter(Boolean);
}
...
function sourceSpecificBody(language, frame, source, article) {
  ...
  if (/hollywood.*bending.*knee.*openai|luca-guadagnino-artificial-sam-altman|artificial.*sam altman/.test(text)) {
    return {
      "zh-Hant": "這個事件線先從 Amazon MGM 放棄發行計畫開始。The Verge 指出，《Artificial》的後期製作接近完成，Amazon 原本被認為會安排短期院線上映以取得奧斯卡資格，之後在 2027 年初擴大上映，並可能安排 SXSW 電影電視節放映。\n\n《Artificial》由 Luca Guadagnino 執導，題材圍繞 OpenAI 共同創辦人兼執行長 Sam Altman。The Verge 報導，Netflix、A24、Focus Features 與 Warner Bros. 旗下 Clockwork 都暫不接手這部片的發行；Neon 和 Mubi 據稱仍然對該片感興趣。\n\nThe Verge 把 Amazon 的決定放在更大的產業脈絡裡看：Amazon 今年稍早宣布對 OpenAI 投資 500 億美元，也明確表態要深度投入 AI 業務。對片商來說，一部以批判角度描寫 AI 產業高層的電影，開始不只是影視內容風險，也會牽動平台、投資與科技合作關係。\n\n報導沒有說《Artificial》已經取消，而是指出發行端變得更謹慎。Neon 或 Mubi 仍可能接手，但幾家大型片商暫不接手，讓這部片成為好萊塢如何處理 AI 產業敘事的一個觀察點。\n\n這則快訊的重點是來源事實本身：Amazon MGM 放手、其他大型片商暫不接手、Neon/Mubi 仍有興趣，以及 The Verge 將它解讀為娛樂業面對大型科技與 AI 題材時的壓力訊號。",
      en: "The story begins with Amazon MGM stepping away from Artificial after the film was already close to finishing post-production. The Verge says Amazon had reportedly been planning a short Oscar-qualifying theatrical run, a wider release in early 2027, and a possible SXSW Film & TV Festival screening.\n\nArtificial is directed by Luca Guadagnino and centers on OpenAI co-founder and CEO Sam Altman. The Verge reports that Netflix, A24, Focus Features, and Warner Bros.-owned Clockwork have passed on distributing it, while Neon and Mubi are still said to be interested.\n\nThe Verge frames the decision inside a larger industry context. Amazon announced a $50 billion OpenAI investment earlier this year and has made clear that it wants a major role in AI, so a film that critically depicts AI industry leadership is no longer only a content bet; it can also touch platform, investment, and technology-partnership politics.\n\nThe report does not say Artificial has been cancelled. It says distribution has become more cautious. Neon or Mubi could still take it on, but several major studios passing on the film makes it a useful signal for how Hollywood handles AI-industry narratives.\n\nThe source-specific takeaway is simple: Amazon MGM stepped away, large studios passed, Neon and Mubi remain possible buyers, and The Verge treats the story as a pressure signal for entertainment companies dealing with big-tech and AI subjects.",
      ja: "この話は、後期制作がほぼ終わりに近づいていた『Artificial』から Amazon MGM が離れたところから始まります。The Verge によると、Amazon は当初、アカデミー賞資格のための短期劇場公開、2027 年初頭の拡大公開、さらに SXSW Film & TV Festival での上映を計画していたとされています。\n\n『Artificial』は Luca Guadagnino が監督し、OpenAI の共同創業者兼 CEO である Sam Altman を中心に描く作品です。The Verge は、Netflix、A24、Focus Features、Warner Bros. 傘下の Clockwork が配給を見送り、Neon と Mubi はまだ関心を持っていると報じています。\n\nThe Verge はこの判断を、より大きな産業文脈に置いています。Amazon は今年、OpenAI への 500 億ドル投資を発表し、AI 事業へ深く関わる姿勢を示しました。そのため、AI 業界のリーダーを批判的に描く映画は、単なるコンテンツの賭けではなく、プラットフォーム、投資、テクノロジー提携にも触れる題材になります。\n\n報道は『Artificial』が中止されたとは述べていません。配給側が慎重になっている、という内容です。Neon や Mubi が引き受ける可能性は残っていますが、大手スタジオが見送ったことは、Hollywood が AI 産業の物語をどう扱うかを見る材料になります。\n\nこの快訊で残すべき事実は、Amazon MGM が離れたこと、大手スタジオが見送ったこと、Neon と Mubi にはまだ可能性があること、そして The Verge がこれを Big Tech と AI 題材を扱うエンタメ企業への圧力信号として読んでいることです。",
      ko: "이 이야기는 후반 작업이 거의 끝나가던 Artificial에서 Amazon MGM이 물러난 데서 시작됩니다. The Verge에 따르면 Amazon은 원래 오스카 자격을 위한 단기 극장 개봉, 2027년 초 확대 개봉, SXSW Film & TV Festival 상영까지 계획했던 것으로 알려졌습니다.\n\nArtificial은 Luca Guadagnino가 연출하고 OpenAI 공동창업자 겸 CEO Sam Altman을 중심으로 다루는 작품입니다. The Verge는 Netflix, A24, Focus Features, Warner Bros. 산하 Clockwork가 배급을 넘겨받지 않기로 했고, Neon과 Mubi는 여전히 관심이 있는 것으로 알려졌다고 보도했습니다.\n\nThe Verge는 이 결정을 더 큰 산업 맥락에 놓습니다. Amazon은 올해 OpenAI에 500억 달러를 투자한다고 발표했고 AI 사업에 깊게 들어가겠다는 신호를 냈습니다. 그래서 AI 업계 리더를 비판적으로 다루는 영화는 단순한 콘텐츠 선택이 아니라 플랫폼, 투자, 기술 협력 관계까지 건드리는 소재가 됩니다.\n\n보도는 Artificial이 취소됐다고 말하지 않습니다. 배급 쪽이 더 조심스러워졌다는 내용입니다. Neon이나 Mubi가 맡을 가능성은 남아 있지만, 여러 대형 스튜디오가 물러났다는 점은 Hollywood가 AI 산업 서사를 어떻게 다루는지 보여주는 신호입니다.\n\n핵심은 출처에 있는 사실입니다. Amazon MGM은 빠졌고, 여러 대형 스튜디오는 통과했으며, Neon과 Mubi는 여전히 가능성이 있고, The Verge는 이를 Big Tech와 AI 소재를 다루는 엔터테인먼트 업계의 압력 신호로 읽습니다.",
      id: "Cerita ini dimulai dari keputusan Amazon MGM mundur dari Artificial ketika post-production film itu hampir selesai. The Verge menyebut Amazon sebelumnya dilaporkan menyiapkan rilis bioskop singkat untuk memenuhi syarat Oscar, rilis lebih luas pada awal 2027, dan kemungkinan pemutaran di SXSW Film & TV Festival.\n\nArtificial disutradarai Luca Guadagnino dan berpusat pada Sam Altman, co-founder sekaligus CEO OpenAI. The Verge melaporkan Netflix, A24, Focus Features, dan Clockwork milik Warner Bros. tidak mengambil distribusinya, sementara Neon dan Mubi masih disebut tertarik.\n\nThe Verge menempatkan keputusan Amazon dalam konteks industri yang lebih luas. Amazon mengumumkan investasi 50 miliar dolar AS ke OpenAI awal tahun ini dan menunjukkan ambisi besar di AI. Karena itu, film yang mengkritisi kepemimpinan industri AI tidak lagi sekadar taruhan konten; ia juga menyentuh platform, investasi, dan hubungan teknologi.\n\nLaporan itu tidak mengatakan Artificial dibatalkan. Isinya adalah distribusi menjadi lebih hati-hati. Neon atau Mubi masih mungkin mengambil film ini, tetapi beberapa studio besar yang lewat membuatnya menjadi sinyal tentang cara Hollywood menangani narasi industri AI.\n\nFakta utamanya tetap dari sumber: Amazon MGM mundur, beberapa studio besar tidak mengambilnya, Neon dan Mubi masih mungkin, dan The Verge membaca cerita ini sebagai sinyal tekanan bagi perusahaan hiburan yang berhadapan dengan Big Tech dan tema AI.",
      vi: "Câu chuyện bắt đầu từ việc Amazon MGM rút khỏi Artificial khi phần hậu kỳ của phim gần hoàn tất. The Verge cho biết Amazon trước đó được cho là dự tính phát hành rạp ngắn để đủ điều kiện Oscar, mở rộng phát hành vào đầu năm 2027 và có thể chiếu tại SXSW Film & TV Festival.\n\nArtificial do Luca Guadagnino đạo diễn, xoay quanh Sam Altman, đồng sáng lập kiêm CEO OpenAI. The Verge đưa tin Netflix, A24, Focus Features và Clockwork thuộc Warner Bros. đều không nhận phát hành phim này, trong khi Neon và Mubi vẫn được cho là còn quan tâm.\n\nThe Verge đặt quyết định của Amazon vào bối cảnh ngành rộng hơn. Amazon đã công bố khoản đầu tư 50 tỷ USD vào OpenAI đầu năm nay và thể hiện rõ tham vọng lớn trong AI. Vì vậy, một bộ phim nhìn phê phán vào lãnh đạo ngành AI không còn chỉ là lựa chọn nội dung; nó còn chạm tới nền tảng, đầu tư và quan hệ hợp tác công nghệ.\n\nBài viết không nói Artificial đã bị hủy. Nó nói phía phát hành đang thận trọng hơn. Neon hoặc Mubi vẫn có thể tiếp nhận, nhưng việc nhiều studio lớn bỏ qua bộ phim trở thành tín hiệu về cách Hollywood xử lý những câu chuyện liên quan đến ngành AI.\n\nĐiểm cần giữ đúng theo nguồn là: Amazon MGM rút lui, nhiều studio lớn không nhận, Neon và Mubi vẫn có khả năng, và The Verge xem đây là tín hiệu áp lực với các công ty giải trí khi phải kể chuyện về Big Tech và AI.",
      th: "เรื่องนี้เริ่มจาก Amazon MGM ถอนตัวจาก Artificial ทั้งที่งาน post-production ของหนังใกล้เสร็จแล้ว The Verge ระบุว่า Amazon เคยถูกคาดว่าจะทำ theatrical run แบบสั้นเพื่อให้เข้าเกณฑ์ Oscar ก่อนขยายการฉายในต้นปี 2027 และอาจนำไปฉายใน SXSW Film & TV Festival\n\nArtificial กำกับโดย Luca Guadagnino และเล่าเรื่องที่เกี่ยวกับ Sam Altman ผู้ร่วมก่อตั้งและ CEO ของ OpenAI The Verge รายงานว่า Netflix, A24, Focus Features และ Clockwork ในเครือ Warner Bros. ต่างไม่รับช่วงจัดจำหน่าย ขณะที่ Neon และ Mubi ยังถูกระบุว่าสนใจอยู่\n\nThe Verge วางการตัดสินใจของ Amazon ไว้ในบริบทอุตสาหกรรมที่กว้างกว่า Amazon ประกาศลงทุน 50 พันล้านดอลลาร์ใน OpenAI เมื่อต้นปี และแสดงชัดว่าต้องการบทบาทใหญ่ใน AI ดังนั้นหนังที่วิจารณ์ผู้นำอุตสาหกรรม AI จึงไม่ใช่แค่การเดิมพันด้านคอนเทนต์ แต่แตะถึงแพลตฟอร์ม การลงทุน และความสัมพันธ์ด้านเทคโนโลยีด้วย\n\nรายงานไม่ได้บอกว่า Artificial ถูกยกเลิก แต่บอกว่าฝั่งจัดจำหน่ายระมัดระวังมากขึ้น Neon หรือ Mubi ยังอาจรับต่อได้ แต่การที่สตูดิโอใหญ่หลายรายผ่านไป ทำให้เรื่องนี้เป็นสัญญาณว่าฮอลลีวูดจัดการเรื่องเล่าเกี่ยวกับอุตสาหกรรม AI อย่างไร\n\nข้อเท็จจริงหลักจากแหล่งข่าวคือ Amazon MGM ถอนตัว สตูดิโอใหญ่หลายรายไม่รับ Neon และ Mubi ยังเป็นไปได้ และ The Verge อ่านเรื่องนี้เป็นสัญญาณแรงกดดันต่อบริษัทบันเทิงที่ต้องรับมือกับ Big Tech และหัวข้อ AI",
      ms: "Kisah ini bermula apabila Amazon MGM menarik diri daripada Artificial ketika post-production filem itu hampir selesai. The Verge menyebut Amazon sebelum ini dilaporkan merancang tayangan pawagam ringkas untuk kelayakan Oscar, tayangan lebih luas pada awal 2027, dan kemungkinan tayangan di SXSW Film & TV Festival.\n\nArtificial diarahkan oleh Luca Guadagnino dan berpusat pada Sam Altman, pengasas bersama serta CEO OpenAI. The Verge melaporkan Netflix, A24, Focus Features dan Clockwork milik Warner Bros. tidak mengambil pengedarannya, manakala Neon dan Mubi masih dikatakan berminat.\n\nThe Verge meletakkan keputusan Amazon dalam konteks industri yang lebih luas. Amazon mengumumkan pelaburan 50 bilion dolar AS dalam OpenAI awal tahun ini dan jelas mahu memainkan peranan besar dalam AI. Maka filem yang mengkritik kepimpinan industri AI bukan lagi sekadar taruhan kandungan; ia juga menyentuh platform, pelaburan dan hubungan teknologi.\n\nLaporan itu tidak mengatakan Artificial dibatalkan. Ia mengatakan bahagian pengedaran menjadi lebih berhati-hati. Neon atau Mubi masih boleh mengambilnya, tetapi beberapa studio besar yang menolak menjadikan filem ini isyarat tentang cara Hollywood mengendalikan naratif industri AI.\n\nFakta utama daripada sumber ialah Amazon MGM berundur, beberapa studio besar tidak mengambilnya, Neon dan Mubi masih berkemungkinan, dan The Verge membaca cerita ini sebagai isyarat tekanan kepada syarikat hiburan yang berdepan Big Tech dan topik AI.",
      fil: "Nagsisimula ang kuwento sa pag-atras ng Amazon MGM sa Artificial kahit halos tapos na ang post-production ng pelikula. Ayon sa The Verge, dati umanong plano ng Amazon ang maikling theatrical run para sa Oscar qualification, mas malawak na release sa unang bahagi ng 2027, at posibleng screening sa SXSW Film & TV Festival.\n\nAng Artificial ay idinirekta ni Luca Guadagnino at nakasentro kay Sam Altman, co-founder at CEO ng OpenAI. Iniulat ng The Verge na hindi kinuha ng Netflix, A24, Focus Features, at Clockwork ng Warner Bros. ang distribution nito, habang interesado pa rin umano ang Neon at Mubi.\n\nInilalagay ng The Verge ang desisyon ng Amazon sa mas malaking konteksto ng industriya. Nag-anunsyo ang Amazon ng 50 bilyong dolyar na investment sa OpenAI ngayong taon at malinaw ang ambisyon nito sa AI. Kaya ang pelikulang kritikal sa leadership ng AI industry ay hindi lang content bet; maaari rin itong tumama sa platform, investment, at technology partnerships.\n\nHindi sinasabi ng report na cancelled na ang Artificial. Ang sinasabi nito ay mas nagiging maingat ang distribution side. Maaari pa rin itong kunin ng Neon o Mubi, pero ang pag-atras ng ilang malalaking studio ay signal kung paano hinahawakan ng Hollywood ang AI-industry narratives.\n\nAng dapat manatiling tumpak sa ulat ay ito: umatras ang Amazon MGM, pumasa ang ilang major studios, posible pa rin ang Neon at Mubi, at binabasa ito ng The Verge bilang pressure signal para sa entertainment companies na humaharap sa Big Tech at AI subjects."
    }[language];
  }
  if (/powering.*next era.*confidential ai|private cloud compute|confidential ai.*apple|pcc.*confidential|titanium.*confidential|intel tdx.*nvidia/i.test(text)) {
    return {
      "zh-Hant": "Google Cloud Blog 這篇文章談的是 Apple 擴大 Private Cloud Compute（PCC）時，背後如何使用 Google Cloud 的機密運算能力。來源指出，Apple 在 WWDC 2026 宣布把 PCC 擴展到更複雜的 AI 推理任務，而這套 serving platform 需要同時滿足安全性、隱私、透明度與可驗證性。\n\n文章提到，Google Cloud 與 Apple 合作建置的架構結合了 Confidential Computing、Titanium 安全架構，以及能在 CPU 到 GPU 推理路徑中保護資料的硬體隔離能力。這裡的重點不是一般雲端代管，而是讓 AI workload 在受保護的 trusted execution environment 裡運行。\n\nGoogle Cloud 也提到，這套平台使用 Intel TDX 與 NVIDIA Confidential Computing，讓推理過程能在 CPU 與 GPU 之間維持更完整的資料保護。對需要處理敏感資料的 AI 服務來說，這代表基礎設施正在從「模型能不能跑」走向「資料在推理期間能不能被隔離、審核與信任」。\n\n來源還強調透明度。PCC 相關 host stack 採開源方式，讓外部安全研究者與使用者能檢查系統行為，而不是只依賴供應商承諾。這點對 confidential AI 很重要，因為企業客戶往往不只問性能，也會問證據、審計與責任邊界。\n\n這則快訊應該被讀成一個基礎設施訊號：Apple 的 PCC、Google Cloud Confidential Computing、Intel TDX、NVIDIA Confidential Computing 與開源驗證一起出現，說明 AI 隱私與推理安全正在成為大型 AI 產品的正式競爭層。",
      en: "The Google Cloud Blog story is about how Apple’s expanded Private Cloud Compute, or PCC, uses Google Cloud confidential-computing capabilities behind the scenes. The source says Apple announced at WWDC 2026 that PCC would support more complex AI inference tasks, and that the serving platform had to meet Apple’s goals for security, privacy, transparency, and verifiability.\n\nGoogle Cloud says the architecture built with Apple combines Confidential Computing, the Titanium security architecture, and hardware isolation that protects data across the CPU-to-GPU inference path. The point is not generic cloud hosting. It is AI workload execution inside a protected trusted execution environment.\n\nThe post also names Intel TDX and NVIDIA Confidential Computing as parts of the design. That matters because sensitive AI services need protection not only while data is stored, but also while inference moves through accelerators and runtime infrastructure.\n\nTransparency is another source detail. Google Cloud says the PCC-related host stack is open sourced so external security researchers and users can inspect system behavior instead of relying only on vendor promises. For confidential AI, that verification layer is part of the product, not a footnote.\n\nThe market signal is infrastructure-level: Apple PCC, Google Cloud Confidential Computing, Intel TDX, NVIDIA Confidential Computing, and open-source verification are appearing in the same stack. AI privacy and inference security are becoming a formal competition layer for large AI products.",
      ja: "Google Cloud Blog の記事は、Apple が Private Cloud Compute（PCC）を拡張する際、その背後で Google Cloud の confidential-computing 機能をどう使っているかを扱っています。出典によると、Apple は WWDC 2026 で PCC をより複雑な AI 推論タスクへ広げると発表し、その serving platform にはセキュリティ、プライバシー、透明性、検証可能性が求められました。\n\nGoogle Cloud は、Apple と構築したアーキテクチャが Confidential Computing、Titanium security architecture、そして CPU から GPU までの推論経路でデータを守るハードウェア分離を組み合わせていると説明しています。これは一般的なクラウドホスティングではなく、AI workload を trusted execution environment の中で動かす話です。\n\n記事では Intel TDX と NVIDIA Confidential Computing も設計要素として挙げられています。機密データを扱う AI サービスでは、保存時だけでなく、推論がアクセラレータや runtime infrastructure を通る間も保護が必要だからです。\n\n透明性も重要な出典事実です。Google Cloud は、PCC 関連の host stack が open source になっており、外部のセキュリティ研究者や利用者がシステム動作を検証できると述べています。confidential AI では、この検証層が製品価値の一部になります。\n\n市場シグナルはインフラ層にあります。Apple PCC、Google Cloud Confidential Computing、Intel TDX、NVIDIA Confidential Computing、open-source verification が同じ stack に入ったことで、AI のプライバシーと推論セキュリティが大型 AI 製品の競争軸になり始めています。",
      ko: "Google Cloud Blog의 이 글은 Apple이 Private Cloud Compute, 즉 PCC를 확장하는 과정에서 Google Cloud의 confidential-computing 역량을 어떻게 쓰는지 다룹니다. 출처에 따르면 Apple은 WWDC 2026에서 PCC를 더 복잡한 AI 추론 작업으로 확대한다고 발표했고, 이 serving platform은 보안, 프라이버시, 투명성, 검증 가능성 목표를 만족해야 했습니다.\n\nGoogle Cloud는 Apple과 함께 만든 아키텍처가 Confidential Computing, Titanium security architecture, 그리고 CPU에서 GPU로 이어지는 추론 경로에서 데이터를 보호하는 하드웨어 격리를 결합한다고 설명합니다. 이는 일반적인 클라우드 호스팅이 아니라 보호된 trusted execution environment 안에서 AI workload를 실행하는 이야기입니다.\n\n글은 Intel TDX와 NVIDIA Confidential Computing도 설계 요소로 언급합니다. 민감한 데이터를 다루는 AI 서비스에서는 저장 중 보호뿐 아니라 추론이 accelerator와 runtime infrastructure를 지나는 동안의 보호도 필요하기 때문입니다.\n\n투명성도 핵심입니다. Google Cloud는 PCC 관련 host stack이 오픈소스로 제공되어 외부 보안 연구자와 사용자가 시스템 동작을 검증할 수 있다고 말합니다. confidential AI에서는 이 검증 계층이 부가 설명이 아니라 제품 신뢰의 일부입니다.\n\n시장 신호는 인프라 레벨에 있습니다. Apple PCC, Google Cloud Confidential Computing, Intel TDX, NVIDIA Confidential Computing, open-source verification이 같은 stack에 등장했다는 것은 AI 프라이버시와 추론 보안이 대형 AI 제품의 공식 경쟁층이 되고 있다는 뜻입니다.",
      id: "Artikel Google Cloud Blog ini membahas bagaimana perluasan Private Cloud Compute, atau PCC, milik Apple memakai kemampuan confidential computing Google Cloud di belakang layar. Sumber menyebut Apple mengumumkan di WWDC 2026 bahwa PCC akan mendukung tugas inferensi AI yang lebih kompleks, sementara serving platform-nya harus memenuhi target keamanan, privasi, transparansi, dan verifiability Apple.\n\nGoogle Cloud menjelaskan arsitektur yang dibangun bersama Apple menggabungkan Confidential Computing, Titanium security architecture, serta isolasi hardware yang melindungi data di jalur inferensi CPU-ke-GPU. Ini bukan sekadar cloud hosting umum, melainkan eksekusi AI workload di dalam trusted execution environment yang terlindungi.\n\nArtikel itu juga menyebut Intel TDX dan NVIDIA Confidential Computing sebagai bagian dari desain. Untuk layanan AI yang menangani data sensitif, perlindungan diperlukan bukan hanya saat data disimpan, tetapi juga ketika inferensi berjalan melewati accelerator dan runtime infrastructure.\n\nDetail lain dari sumber adalah transparansi. Google Cloud mengatakan host stack terkait PCC dibuat open source agar peneliti keamanan eksternal dan pengguna dapat memeriksa perilaku sistem, bukan hanya bergantung pada janji vendor. Dalam confidential AI, lapisan verifikasi ini adalah bagian dari produk.\n\nSinyal pasarnya berada di level infrastruktur: Apple PCC, Google Cloud Confidential Computing, Intel TDX, NVIDIA Confidential Computing, dan open-source verification muncul dalam satu stack. Privasi AI dan keamanan inferensi sedang menjadi lapisan kompetisi resmi untuk produk AI besar.",
      vi: "Bài viết trên Google Cloud Blog nói về cách Apple mở rộng Private Cloud Compute, hay PCC, và dùng năng lực confidential computing của Google Cloud ở phía sau. Nguồn cho biết Apple công bố tại WWDC 2026 rằng PCC sẽ hỗ trợ các tác vụ suy luận AI phức tạp hơn, còn serving platform phải đáp ứng mục tiêu về bảo mật, quyền riêng tư, tính minh bạch và khả năng kiểm chứng.\n\nGoogle Cloud mô tả kiến trúc xây cùng Apple kết hợp Confidential Computing, Titanium security architecture và lớp cô lập phần cứng để bảo vệ dữ liệu trên đường suy luận từ CPU sang GPU. Đây không phải câu chuyện cloud hosting thông thường, mà là chạy AI workload trong một trusted execution environment được bảo vệ.\n\nBài viết cũng nhắc Intel TDX và NVIDIA Confidential Computing như một phần của thiết kế. Với dịch vụ AI xử lý dữ liệu nhạy cảm, dữ liệu cần được bảo vệ không chỉ khi lưu trữ mà cả khi inference đi qua accelerator và runtime infrastructure.\n\nMinh bạch là chi tiết quan trọng khác. Google Cloud cho biết host stack liên quan đến PCC được open source để nhà nghiên cứu bảo mật bên ngoài và người dùng có thể kiểm tra hành vi hệ thống, thay vì chỉ dựa vào cam kết của nhà cung cấp. Với confidential AI, lớp kiểm chứng này là một phần của sản phẩm.\n\nTín hiệu thị trường nằm ở tầng hạ tầng: Apple PCC, Google Cloud Confidential Computing, Intel TDX, NVIDIA Confidential Computing và open-source verification xuất hiện trong cùng một stack. Quyền riêng tư AI và bảo mật inference đang trở thành lớp cạnh tranh chính thức của các sản phẩm AI lớn.",
      th: "บทความของ Google Cloud Blog พูดถึงการขยาย Private Cloud Compute หรือ PCC ของ Apple และการใช้ความสามารถด้าน confidential computing ของ Google Cloud เบื้องหลัง แหล่งข่าวระบุว่า Apple ประกาศใน WWDC 2026 ว่า PCC จะรองรับงาน AI inference ที่ซับซ้อนขึ้น และ serving platform ต้องตอบโจทย์ security, privacy, transparency และ verifiability ของ Apple\n\nGoogle Cloud อธิบายว่าสถาปัตยกรรมที่สร้างร่วมกับ Apple ผสาน Confidential Computing, Titanium security architecture และการแยกด้วยฮาร์ดแวร์เพื่อปกป้องข้อมูลตลอดเส้นทาง inference จาก CPU ไป GPU ประเด็นจึงไม่ใช่ cloud hosting ทั่วไป แต่เป็นการรัน AI workload ใน trusted execution environment ที่ถูกป้องกัน\n\nบทความยังระบุ Intel TDX และ NVIDIA Confidential Computing เป็นส่วนหนึ่งของการออกแบบ สำหรับบริการ AI ที่จัดการข้อมูลอ่อนไหว การปกป้องต้องเกิดไม่ใช่แค่ตอนจัดเก็บข้อมูล แต่รวมถึงช่วงที่ inference วิ่งผ่าน accelerator และ runtime infrastructure ด้วย\n\nอีกประเด็นจากแหล่งข่าวคือ transparency Google Cloud ระบุว่า host stack ที่เกี่ยวข้องกับ PCC เป็น open source เพื่อให้นักวิจัยความปลอดภัยภายนอกและผู้ใช้ตรวจสอบพฤติกรรมระบบได้ แทนที่จะเชื่อคำสัญญาของ vendor อย่างเดียว ใน confidential AI ชั้นการตรวจสอบนี้เป็นส่วนหนึ่งของตัวผลิตภัณฑ์\n\nสัญญาณตลาดอยู่ที่ระดับโครงสร้างพื้นฐาน: Apple PCC, Google Cloud Confidential Computing, Intel TDX, NVIDIA Confidential Computing และ open-source verification อยู่ใน stack เดียวกัน แปลว่า privacy ของ AI และความปลอดภัยของ inference กำลังกลายเป็นชั้นการแข่งขันทางการของผลิตภัณฑ์ AI ขนาดใหญ่",
      ms: "Artikel Google Cloud Blog ini menerangkan bagaimana peluasan Private Cloud Compute, atau PCC, Apple menggunakan keupayaan confidential computing Google Cloud di belakang tabir. Sumber itu menyebut Apple mengumumkan di WWDC 2026 bahawa PCC akan menyokong tugas inferens AI yang lebih kompleks, dan serving platform perlu memenuhi matlamat keselamatan, privasi, ketelusan serta kebolehsemakan Apple.\n\nGoogle Cloud berkata seni bina yang dibina bersama Apple menggabungkan Confidential Computing, Titanium security architecture, serta pengasingan hardware yang melindungi data sepanjang laluan inferens CPU-ke-GPU. Ini bukan cloud hosting biasa, tetapi pelaksanaan AI workload dalam trusted execution environment yang dilindungi.\n\nArtikel itu turut menamakan Intel TDX dan NVIDIA Confidential Computing sebagai sebahagian daripada reka bentuk. Untuk perkhidmatan AI yang mengendalikan data sensitif, perlindungan diperlukan bukan hanya ketika data disimpan, tetapi juga semasa inferens bergerak melalui accelerator dan runtime infrastructure.\n\nKetelusan ialah satu lagi butiran penting daripada sumber. Google Cloud menyebut host stack berkaitan PCC bersifat open source supaya penyelidik keselamatan luar dan pengguna boleh memeriksa tingkah laku sistem, bukan sekadar bergantung pada janji vendor. Dalam confidential AI, lapisan verifikasi ini ialah sebahagian daripada produk.\n\nIsyarat pasaran berada pada tahap infrastruktur: Apple PCC, Google Cloud Confidential Computing, Intel TDX, NVIDIA Confidential Computing dan open-source verification muncul dalam stack yang sama. Privasi AI dan keselamatan inferens sedang menjadi lapisan persaingan rasmi untuk produk AI besar.",
      fil: "Tungkol ang Google Cloud Blog story sa kung paano ginagamit ng pinalawak na Private Cloud Compute, o PCC, ng Apple ang confidential-computing capabilities ng Google Cloud sa likod nito. Ayon sa source, inanunsyo ng Apple sa WWDC 2026 na susuportahan ng PCC ang mas kumplikadong AI inference tasks, at kailangang maabot ng serving platform ang security, privacy, transparency, at verifiability goals ng Apple.\n\nSabi ng Google Cloud, ang architecture na binuo kasama ang Apple ay pinagsasama ang Confidential Computing, Titanium security architecture, at hardware isolation na nagpoprotekta ng data sa CPU-to-GPU inference path. Hindi ito ordinaryong cloud hosting. Ito ay pagpapatakbo ng AI workload sa loob ng protected trusted execution environment.\n\nBinanggit din ng post ang Intel TDX at NVIDIA Confidential Computing bilang bahagi ng design. Para sa AI services na humahawak ng sensitibong data, kailangang protektado ang data hindi lang habang naka-store, kundi habang dumadaan ang inference sa accelerators at runtime infrastructure.\n\nIsa pang mahalagang detalye ang transparency. Sabi ng Google Cloud, open source ang PCC-related host stack para masuri ng external security researchers at users ang system behavior, sa halip na umasa lang sa pangako ng vendor. Sa confidential AI, bahagi ng produkto ang verification layer na ito.\n\nAng market signal ay nasa infrastructure level: Apple PCC, Google Cloud Confidential Computing, Intel TDX, NVIDIA Confidential Computing, at open-source verification ay nasa iisang stack. Nagiging opisyal na competition layer ng malalaking AI products ang AI privacy at inference security."
    }[language];
  }
  if (/introducing-openai-partner-network|openai partner network|partner network/.test(text)) {
    return {
