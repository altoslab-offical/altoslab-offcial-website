...
const aiFeelingAudit = read("scripts/blog-audit-ai-feeling.mjs");
...
assert(quality.includes("marketNewsCopyIssues"), "quality gate applies market-news public-copy hard checks before release");
assert(quality.includes("generic ALTOS LAB advice filler"), "quality gate blocks generic market-news advice filler");
assert(quality.includes("non-English market news must not paste the full English source title"), "quality gate blocks raw English source-title leaks in localized market news");
assert(localWorker.includes("marketNewsCopyGateIssues"), "local worker preflight blocks weak market-news copy before calling production ingest");
...
assert(quality.includes("recycledColumnBodyPatterns"), "blog quality gate blocks recycled column body scaffolding");
assert(quality.includes("repeatedColumnTakeawayPatterns"), "blog quality gate blocks generic repeated column takeaways");
...
assert(marketSourceScanner.includes("exclude-source-urls"), "market source scanner can exclude already-attempted source URLs during fill loops");
...
assert(scheduledRunner.includes("excludeSourceUrls") && scheduledRunner.includes("attemptedSourceUrls"), "market fill carries attempted source URLs across scan attempts");
...
assert(sopDoctor.includes("marketNewsCopyGateIssues"), "SOP doctor independently blocks weak market-news copy in prepared article sets");
assert(packageJson.includes("\"blog:sop-doctor\""), "package scripts expose blog:sop-doctor alias for SOP runbooks");
...
assert(envExample.includes("BLOG_MARKET_TRANSLATION_PROVIDER=google-web"), "env example keeps market-news translation on the source-faithful web translation provider by default");
...
assert(marketTranslationService.includes('BLOG_MARKET_HERMES_ALLOW_DETERMINISTIC_SOURCE_TRANSLATION === "1"'), "market-news Hermes deterministic fallback requires explicit opt-in");
assert(marketTranslationService.includes('BLOG_MARKET_TRANSLATION_PROVIDER || "google-web"'), "market-news translation service defaults away from deterministic Hermes fallback");
assert(scheduledRunner.includes('BLOG_MARKET_TRANSLATION_PROVIDER_OVERRIDE ||\n          "google-web"'), "scheduled market source worker defaults away from deterministic Hermes fallback");
assert(scheduledRunner.includes('BLOG_MARKET_HERMES_ALLOW_DETERMINISTIC_SOURCE_TRANSLATION || "0"'), "scheduled market source worker keeps deterministic fallback disabled by default");
assert(marketSourceWorker.includes('return "source-translation"') && marketSourceWorker.includes("translationProvider"), "market source worker keeps approved source-translation provider while recording google-web provenance");
