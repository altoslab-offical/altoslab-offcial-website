...
export function reviewImageFit(post: BlogPost): ReviewResult {
  ...
  if (!post.cover || !post.coverAlt) {
    ...
    if (post.contentType === "breaking" && post.coverSource !== "source") {
      issues.push("market news posts must use the credited source article or official announcement image");
    }
  }
...
function reviewPost(post: BlogPost, multilingual: ReviewResult): PostReview {
  ...
  if (post.contentType === "breaking" && post.coverSource !== "source") {
    issues.push("market news production covers must come from the source article or official announcement image lane before release");
  }
